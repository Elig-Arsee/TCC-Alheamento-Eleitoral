...
DescTools::PseudoR2(mod_satisfação, which = "Nagelkerke")
...
gtsummary::tbl_regression(mod_satisfação, exponentiate = TRUE)
help("tbl_regression")

install.packages("pacman", repos="http://cran.rstudio.com/", dependencies=TRUE)

#Gráfico
sjPlot::plot_model(mod_satisfação)
...
#ANÁLISE CONFIANÇA
#Checagem de pressupostos Confiança

#1.Variavel dependete nominal com categorias mutamente exclusivas
#VD: Q12P2.B_Atitude_Turno_2. Categorias: Abstenção, Voto anulado, Voto em branco, Voto nominal - Ok

#2. Independência das observações (sem medidas repetidas)
#Cada linha é uma pessoa - Ok.

#3. Ausência de multicolinearidade (alta correlação entre variáveis independentes do modelo)
#3.1 Checagem de correlação entre variáveis independentes
psych::pairs.panels(Data2018[14:17])#somente variáveis confiança

#3.1. Checagem de Vif (correlação entre variáveis independentes:
#quando vif está acima de 10 tem problema nos pressupostos)
#Construção de modelo futuro: voto foi transformado em variável numérica,
#mas isso não influencia pois o teste é realizado somente para avaliar correlação entre variáveis independentes)

modelo_conf <- lm(as.numeric(Q12P2.B_Atitude_Turno_2)~ P5_Confianca_Eleicoes+P4.4_Confianca_Governo_Federal+P4.7_Confianca_Partidos_Politicos+P4.8_Confianca_Congresso_Nacional_Senado_CamaraDeputados,
                  data = Data2018)

car::vif(modelo_conf)

#4. Independência de alternativas irrelevantes (teste Hausman-MCFaddem)
#(verifica se, se uma das opções não existisse (voto nulo, branco, abstenção ou NR/NS),
#o coeficiente seria o mesmo, ou seja, não haveria impacto sobre a decisão do voto.É um teste muito criticado.

modiib <- mlogit::mlogit(Q12P2.B_Atitude_Turno_2 ~ 1 | P5_Confianca_Eleicoes +
...
#modelo excluindo voto em branco
modiib2 <- mlogit::mlogit(Q12P2.B_Atitude_Turno_2 ~ 1 | P5_Confianca_Eleicoes +
                            P4.4_Confianca_Governo_Federal +
                            P4.7_Confianca_Partidos_Politicos +
                            P4.8_Confianca_Congresso_Nacional_Senado_CamaraDeputados,
                          data = Data2018,
...
#modelo excluindo voto nulo
modiib3 <- mlogit::mlogit(Q12P2.B_Atitude_Turno_2 ~ 1 | P5_Confianca_Eleicoes +
                            P4.4_Confianca_Governo_Federal +
                            P4.7_Confianca_Partidos_Politicos +
                            P4.8_Confianca_Congresso_Nacional_Senado_CamaraDeputados,
                          data = Data2018,
...
modiib4 <- mlogit::mlogit(Q12P2.B_Atitude_Turno_2 ~ 1 | P5_Confianca_Eleicoes +
                            P4.4_Confianca_Governo_Federal +
                            P4.7_Confianca_Partidos_Politicos +
                            P4.8_Confianca_Congresso_Nacional_Senado_CamaraDeputados,
                          data = Data2018,
...
#comparando modelos para ver se há independência das alternativas irrelevantes (H0), p precisa ser maior que 0,05
mlogit::hmftest(modiib, modiib2)
mlogit::hmftest(modiib, modiib3)
mlogit::hmftest(modiib, modiib4)
...
#Ajuste do modelo (H0: Modelo nulo é igual ao modelo construído, caso hipótese seja confirmada,
#significa que o modelo construído é inutil, p precisa ser menor que 0,05)
anova(mod_conf0, mod_conf)

#Verificação do r² (proporção da VD que é explicada pelas VI)
...
#Tabela completa
gtsummary::tbl_regression(mod_conf, exponentiate = TRUE)
